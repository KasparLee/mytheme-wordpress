...
///////////////
	...
	// SMOOTH SCROLLING (Scroll to section slowly, rather than instanty (default))
	function smooth_scroll(id) {
		if ($(id).length) {
			scrolling = true;
			$('html, body').animate(
				{ scrollTop: $(id).offset().top + 1 },
				{ duration: 500 + ($(id).offset().top / 2),
					complete: function() {
	...
	// CHECK IF ELEMENT IS IN VIEW
		...
		function isiv3(elem, pixels) {
			if (elem.length) {
			var pageBottom = $(window).scrollTop() + $(window).height(); // Bottom of the page
			var elemBottom = $(elem).offset().top + $(elem).height(); // Bottom of the element
			var inView = elemBottom + pixels < pageBottom; // If the Element Bottom + pixels is above the Page bottom
    		return (inView); // Return true or false
    		}
		}
	...
	function animate() {
		...
		$('.featureimgcont').each(function(index, element) {
			// If element is in veiwport, animate
...
///////////////
	...
	var scrolling = false;
	animate_var = function() { animate() } // To access the animate() function globally
...
///////////////
	...
	// Animate elements when page moves, changes or custom events to animate are triggered
	$(window).on('load scroll touchmove resize', function() { if (!scrolling) { animate() } })
